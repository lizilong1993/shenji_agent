...
│   ├── ai/                       # AI开发模块
│   │   ├── __init__.py          # 导出Agent类
│   │   ├── base_agent.py        # AI抽象基类（必须继承）
│   │   ├── agent.py             # DemoAI实现（参考示例）
│   │   └── map.py               # 地图工具类（寻路等）
│   ├── Data/                     # 地图和想定数据
│   │   ├── maps/                # 地图数据
│   │   └── scenarios/           # 想定数据
...
- **AI算法层**：`land_wargame_sdk/ai/` 下的 `BaseAgent` 抽象接口与 `Agent` 示例实现，开发者主要在此扩展策略。
...
推演结束后会在 `logs/replays/` 目录生成复盘文件。

---

## 开发自己的AI

1. 修改 `land_wargame_sdk/ai/agent.py` 中的 `Agent` 类
2. 或创建新的Agent类继承 `BaseAgent`
3. 实现 `setup()`、`step()`、`reset()` 三个方法
4. 在 `run_offline_games.py` 中导入你的Agent并运行
...
| 宿主机路径 | 容器路径 | 用途 |
|------------|----------|------|
| `./land_wargame_sdk/ai` | `/workspace/ai` | AI代码（只读挂载，实时同步） |
